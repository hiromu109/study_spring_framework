...
public class MyDbFirstController {
	...
	@RequestMapping(path = "/mydbfirst", method = RequestMethod.POST)
	public String db(String kensaku, Model model) {

		//検索処理
		List<Map<String,Object>> kensakupra = jdbcTemplate.queryForList("SELECT * FROM firstpratable where title = ?",kensaku);
	...
	public String dbfgpra(Model model) {
		...
		//検索処理
		List<Map<String,Object>> kensakupra = jdbcTemplate.queryForList("SELECT * FROM firstpratable where title = 'おにぎり'");
		//検索結果のリストをmodelに格納する。
		model.addAttribute("kensakupra",kensakupra);
